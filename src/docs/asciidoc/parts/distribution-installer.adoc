...
This is the approach to follow if you need suppotr for unpacking MSIs. There is a helper method called link:{groovydoc}/++AbstractDistributionInstaller.html#unpackMSI(java.io.File, java.io.File)++[unpackMSI] which calls out to the Microsoft utility `msiexec` with the correct parameters. In order to use htis in a practical way it is better to override the `unpack` method and call it from there. For example:
